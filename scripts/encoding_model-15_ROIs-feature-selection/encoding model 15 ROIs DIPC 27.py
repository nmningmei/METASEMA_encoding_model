...
for the computer vision models, 300 best features were selected
...
here                    = 'encoding_model_15_ROIs_with_feature_selection'
...
sub                     = 'DANIEL'# star means all subjects
...
for fmri,csv in zip(working_fmri,working_data):
    ...
    #                idxs_train      = [utils.check_train_balance(df_data,idx_train,list(label_map.keys())) for idx_train in tqdm(idxs_train)]
                ...
                print(word2vec_name,f'{mean_variance.mean():.4f},{aic.mean():.2f},{cp.mean():.2f}')
                # saving the results
                results                     = defaultdict()
        ...
        for img2vec_vec,img2vec_name in zip(image2vec_vecs,image2vec_names):
            ...
            if csv_filename in processed: # don't repeat what have been done
                ...
                # compute mutual information of the train set of each fold
                mutual = []
                ...
                # pick-up the indices of the 300-best features in each fold
                temp1,temp2 = [],[]
                for idx_train,idx_test,mutual_info in zip(idxs_train,idxs_test,mutual):
                    temp1.append((idx_train,np.array(list(mutual_info.nlargest(300).index))))
                    temp2.append((idx_test,np.array(list(mutual_info.nlargest(300).index))))
                ...
                # white box cross validation
                n_coef          = 300#embedding_features.shape[1]
                ...
                positive_voxels = np.array([np.sum(temp >= 0) for temp in scores])
                determinant     = np.array([1 - np.sum((BOLD[idx_test] - y_pred)**2) / np.sum((BOLD[idx_test] - BOLD[idx_test].mean(0))**2) for idx_test,y_pred in zip(idxs_test,preds)])
                print(img2vec_name,f'{mean_variance.mean():.4f},{aic.mean():.2f},{cp.mean():.2f}')
                # saving the results
                results                     = defaultdict()
                results['sub_name'         ]= [sub_name] *          n_splits
                results['roi_name'         ]= [roi_name] *          n_splits
